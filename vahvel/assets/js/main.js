...
$(document).on('click', '.classbtn', function() {
	...
	$.getJSON( "ajax/users.php", {'ChooseClass':"1","class":class_id}, function( data ) {
		location.href = data.url;
	});	
});
...
$('.showTab a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
  ...
  $.getJSON( "ajax/users.php", {'getSubjects':"1","what":id}, function( data ) {
	$(id).html(data.title);	  
